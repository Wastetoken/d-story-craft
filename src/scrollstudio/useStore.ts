...
import { StoreState, SceneConfig, SceneChapter, StorySection, Hotspot, StorySectionStyle, ProjectSchema, Keyframe, TransitionConfig, AssetAudit, PerformanceTier, MaterialOverride, FontDefinition, ViewMode, EngineMode } from './types';
...
export const useStore = create<StoreState & {
  ...
  setMode: (mode) => set({ mode, viewMode: 'cinema' }),
  ...
  addChapter: (modelUrl, name) => set((state) => {
    ...
    return { chapters: newChapters, activeChapterId: id, engineError: null };
  }),
  ...
  removeChapter: (id) => set((state) => {
    ...
    return { chapters: filtered, activeChapterId: newActiveId, engineError: filtered.length === 0 ? null : state.engineError };
  }),
  ...
  updateMaterial: (meshName, updates) => set((state) => ({
    chapters: state.chapters.map(c => {
      ...
      return {
        ...c,
        materialOverrides: { ...c.materialOverrides, [meshName]: { ...current, ...updates } }
      };
    })
  })),
  ...
  addFont: (font) => set((state) => ({
    typography: { ...state.typography, fonts: [...state.typography.fonts, font] }
  })),

  removeFont: (fontId) => set((state) => ({
    typography: { ...state.typography, fonts: state.typography.fonts.filter(f => f.id !== fontId) }
  })),
  ...
  reset: () => set(() => ({
    mode: 'edit' as EngineMode,
    viewMode: 'cinema' as ViewMode,
